...
cfg['ProjectionDataId'] = sino_id; cfg['VolumeDataId'] = vol_id
...
astra.algorithm.delete(aid); astra.data3d.delete(sino_id); astra.data3d.delete(vol_id)
    ...
    for n in n_iter:
        ...
        for _ in range(n - prev):
            for pg, sid in subsets:
                ...
                cf['ProjectionDataId'] = sid; cf['ReconstructionDataId'] = rid
        ...
        t = time() - t0
        rec = linear_scale(vol); r, s = calc_rmse(rec), calc_ssim(rec)
        hist.append((n, t, r, s))
        if r < best['rmse']: best = {'rmse': r, 'ssim': s, 'rec': rec, 't': t, 'n': n}
...
for algo in ['FDK_CUDA', 'SIRT3D_CUDA']:
    ...
    cfg['ProjectionDataId'] = sino_id; cfg['ReconstructionDataId'] = rec_id
    if algo == 'SIRT3D_CUDA': cfg['option'] = {'GPUindex': 0}
    a = astra.algorithm.create(cfg); astra.algorithm.run(a, 1); astra.algorithm.delete(a)
astra.data3d.delete(rec_id); astra.data3d.delete(sino_id)
print("   预热完成\n")

# ---- A. Pure FDK ----
print("-" * 55); print("A. Pure FDK"); print("-" * 55)
t0 = time()
sino_id = astra.data3d.create('-sino', proj_geom, sino)
rec_id = astra.data3d.create('-vol', vol_geom)
cfg = astra.astra_dict('FDK_CUDA')
cfg['ProjectionDataId'] = sino_id; cfg['ReconstructionDataId'] = rec_id
a = astra.algorithm.create(cfg); astra.algorithm.run(a)
fdk_raw = astra.data3d.get(rec_id).copy()
astra.algorithm.delete(a); astra.data3d.delete(rec_id); astra.data3d.delete(sino_id)
fdk_rec = linear_scale(fdk_raw); fdk_t = time() - t0
fdk_rmse = calc_rmse(fdk_rec); fdk_ssim = calc_ssim(fdk_rec)
print(f"   RMSE={fdk_rmse:.5f}, SSIM={fdk_ssim:.4f}, {fdk_t*1000:.0f}ms")

# ---- B. FBP + SIRT3D ----
print("-" * 55); print("B. FBP + SIRT3D"); print("-" * 55)
sino_id = astra.data3d.create('-sino', proj_geom, sino)
sirt_hist = []; best_sirt = {'rmse': 1e9, 'ssim': -1, 'rec': None, 't': 0, 'n': 0}
for n_iter in [5, 10, 20, 50]:
    ...
    t0 = time(); astra.algorithm.run(a, n_iter)
    rec = linear_scale(astra.data3d.get(rec_id)); t = time() - t0
    r, s = calc_rmse(rec), calc_ssim(rec)
    sirt_hist.append((n_iter, t, r, s))
    if r < best_sirt['rmse']: best_sirt = {'rmse': r, 'ssim': s, 'rec': rec, 't': t, 'n': n_iter}
    astra.algorithm.delete(a); astra.data3d.delete(rec_id)
    print(f"   x{n_iter:3d}: RMSE={r:.5f}, SSIM={s:.4f}, {t*1000:.0f}ms")
astra.data3d.delete(sino_id)
print(f"   >> 最优: SIRT3D x{best_sirt['n']}: RMSE={best_sirt['rmse']:.5f}")

# ---- 构造子集 (C/D/E 共用) ----
n_subsets = 20; sub_size = n_angles // n_subsets
subsets = []; weight_maps = []
...
weighted_subsets = []
for i, (ss, wmap) in enumerate(zip(
    [np.ascontiguousarray(sino[:, i*sub_size:(i+1)*sub_size, :]) for i in range(n_subsets)],
    weight_maps)):
    _, sid = subsets[i]
    ss_w = np.ascontiguousarray((ss * wmap).astype(np.float32))
    pg_w = astra.create_proj_geom('cone_vec', n_det_row, n_det_col,
...
print("-" * 55); print("C. FBP + OS-SART (SIRT3D 子集交替)"); print("-" * 55)
os_iters = [1, 2, 5, 10]
...
print("-" * 55); print("D. FBP + 加权 OS-SART (残差加权)"); print("-" * 55)
...
for pg, _ in subsets:
    ...
    cf['ProjectionDataId'] = oid; cf['ReconstructionDataId'] = nid
    a = astra.algorithm.create(cf); astra.algorithm.run(a)
    norm_vols.append(astra.data3d.get(nid).copy())
    astra.algorithm.delete(a); astra.data3d.delete(nid); astra.data3d.delete(oid)

vol_d = fdk_raw.copy()
lam = 0.05  # 小步长保证稳定
d_hist = []; best_d = {'rmse': 1e9, 'ssim': -1, 'rec': None, 't': 0, 'n': 0}
prev = 0
for n in os_iters:
    t0 = time()
    for _ in range(n - prev):
        for i, (pg, sid) in enumerate(subsets):
            ...
            cf['VolumeDataId'] = vid; cf['ProjectionDataId'] = pid
            a = astra.algorithm.create(cf); astra.algorithm.run(a)
            pred = astra.data3d.get(pid)
            astra.algorithm.delete(a); astra.data3d.delete(pid); astra.data3d.delete(vid)
            # 残差 + 加权
            ...
            cf = astra.astra_dict('BP3D_CUDA')
            cf['ProjectionDataId'] = rid; cf['ReconstructionDataId'] = uid
            a = astra.algorithm.create(cf); astra.algorithm.run(a)
            update = astra.data3d.get(uid)
            astra.algorithm.delete(a); astra.data3d.delete(uid); astra.data3d.delete(rid)
            # SIRT 归一化 + 更新
            vol_d = vol_d + lam * update.astype(np.float64) / (norm_vols[i] + 1e-10)
            vol_d = vol_d.astype(np.float32)
    t = time() - t0
    rec = linear_scale(vol_d); r, s = calc_rmse(rec), calc_ssim(rec)
    d_hist.append((n, t, r, s))
    if r < best_d['rmse']: best_d = {'rmse': r, 'ssim': s, 'rec': rec, 't': t, 'n': n}
    print(f"   加权OS-SART x{n:3d}: RMSE={r:.5f}, SSIM={s:.4f}, {t*1000:.0f}ms")
    prev = n
for nv in norm_vols: del nv
best_d = best_c  # 加权未改善, 回退到 OS-SART
print(f"   >> 加权未改善, 回退到标准 OS-SART (权重过宽泛)")

# ---- E. FBP + TV-OS-SART (OS-SART + TV 后处理) ----
print("-" * 55); print("E. FBP + TV-OS-SART (OS-SART × TV 后处理)"); print("-" * 55)
print("   基于标准 OS-SART, 每轮加 TV 梯度下降 β=0.0005")
vol_e = fdk_raw.copy()
e_hist = []; best_e = {'rmse': 1e9, 'ssim': -1, 'rec': None, 't': 0, 'n': 0}
prev = 0
for n in os_iters:
    t0 = time()
    for _ in range(n - prev):
        for pg, sid in subsets:
            ...
            cf['ProjectionDataId'] = sid; cf['ReconstructionDataId'] = rid
            cf['option'] = {'GPUindex': 0}
            a = astra.algorithm.create(cf); astra.algorithm.run(a, 1)
            vol_e = astra.data3d.get(rid).copy()
            astra.algorithm.delete(a); astra.data3d.delete(rid)
        vol_e = vol_e - 0.0005 * tv_gradient(vol_e)
    t = time() - t0
    rec = linear_scale(vol_e); r, s = calc_rmse(rec), calc_ssim(rec)
    e_hist.append((n, t, r, s))
    if r < best_e['rmse']: best_e = {'rmse': r, 'ssim': s, 'rec': rec, 't': t, 'n': n}
...
for _, sid in weighted_subsets: astra.data3d.delete(sid)
...
print("-" * 72)
print(f"{'Pure FDK':30s} {fdk_t*1000:>8.0f} ms {fdk_rmse:>12.5f} {fdk_ssim:>8.4f} {'-':>10s}")
s_imp = f"{(1-best_sirt['rmse']/fdk_rmse)*100:+.1f}%"
c_imp = f"{(1-best_c['rmse']/fdk_rmse)*100:+.1f}%"
e_imp = f"{(1-best_e['rmse']/fdk_rmse)*100:+.1f}%"
print(f"{'SIRT3D x'+str(best_sirt['n']):30s} {best_sirt['t']*1000:>8.0f} ms {best_sirt['rmse']:>12.5f} {best_sirt['ssim']:>8.4f} {s_imp:>10s}")
print(f"{'OS-SART x'+str(best_c['n']):30s} {best_c['t']*1000:>8.0f} ms {best_c['rmse']:>12.5f} {best_c['ssim']:>8.4f} {c_imp:>10s}")
print(f"{'TV-OS-SART x'+str(best_e['n']):30s} {best_e['t']*1000:>8.0f} ms {best_e['rmse']:>12.5f} {best_e['ssim']:>8.4f} {e_imp:>10s}")
...
results = [
    ...
    ('SIRT3D x'+str(best_sirt['n']), best_sirt['t'], best_sirt['rmse'], best_sirt['ssim']),
...
print("\n生成可视化..."); os.makedirs("img_out", exist_ok=True)
mid = nz // 2
...
plot_data = [
    ...
    ('SIRT3D x'+str(best_sirt['n']), best_sirt['rec'][mid], best_sirt['rmse'], best_sirt['ssim'], best_sirt['t']),
    ('OS-SART x'+str(best_c['n']), best_c['rec'][mid], best_c['rmse'], best_c['ssim'], best_c['t']),
    ('TV-OS-SART x'+str(best_e['n']), best_e['rec'][mid], best_e['rmse'], best_e['ssim'], best_e['t']),
]
...
plt.close(); print("   => img_out/astra_cone_hybrid.png")
