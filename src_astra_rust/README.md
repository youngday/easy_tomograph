...
├── c_api/
│   ├── astra_c_api.cpp        # C shim: 包装 ASTRA C++/CUDA 为 extern "C"
...
零第三方 crate(纯 std):构建不需网络。GPU 内核无法在 Rust 中重写,故经 FFI
调用 libastra(与 C++ 相同),**结果逐位一致**是必然且可验证的。
...
耗时 (GTX 1660): TV-OS-SART ~1.9s, Hybrid ~2.5s — 与 C++ (~1.65s / ~2.3s) 相当,
略慢 ~15% (Rust FFI 层每 epoch 一次 33MB 拷贝 + 闭包克隆), 大头仍是 GPU 内核。
