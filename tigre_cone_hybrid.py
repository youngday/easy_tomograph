...
Z, Y, X = np.ogrid[:nz, :N, :N]; cz, cy, cx = nz/2, N/2, N/2
body = ((Z-cz)/12)**2 + ((Y-cy)/(N*0.42))**2 + ((X-cx)/(N*0.35))**2 <= 1; vol_gt[body] = 0.020
...
organ = ((Z-cz+4)/6)**2 + ((Y-cy-15)/(N*0.12))**2 + ((X-cx+10)/(N*0.10))**2 <= 1; vol_gt[organ] = 0.025
tumor = ((Z-cz-3)/4)**2 + ((Y-cy+20)/(N*0.06))**2 + ((X-cx-15)/(N*0.06))**2 <= 1; vol_gt[tumor] = 0.035
air = ((Z-cz+2)/5)**2 + ((Y-cy+25)/(N*0.08))**2 + ((X-cx+25)/(N*0.06))**2 <= 1; vol_gt[air] = 0.0
...
def tv_gradient(vol, eps=1e-8):
    dx = np.zeros_like(vol); dy = np.zeros_like(vol); dz = np.zeros_like(vol)
    dx[:,:,:-1] = vol[:,:,1:] - vol[:,:,:-1]
    dy[:,:-1,:] = vol[:,1:,:] - vol[:,:-1,:]
    dz[:-1,:,:] = vol[1:,:,:] - vol[:-1,:,:]
...
fdk_rec = linear_scale(rec_fdk); fdk_t = time() - t0
fdk_rmse = calc_rmse(fdk_rec); fdk_ssim = calc_ssim(fdk_rec)
print(f"   RMSE={fdk_rmse:.5f}, SSIM={fdk_ssim:.4f}, {fdk_t*1000:.0f}ms")

# ---- B. FBP + SIRT ----
print("-" * 55); print("B. FBP + SIRT"); print("-" * 55)
b_hist = []; best_b = {'rmse': 1e9, 'ssim': -1, 'rec': None, 't': 0, 'n': 0}
rec_b = rec_fdk.copy()
for n_iter in [5, 10, 20, 50]:
    t0 = time(); rec_b = algs.sirt(sino, geo, angles, niter=n_iter, init=rec_b, noneg=False)
    t = time() - t0; rec2d = linear_scale(rec_b)
    r, s = calc_rmse(rec2d), calc_ssim(rec2d)
    b_hist.append((n_iter, t, r, s))
    if r < best_b['rmse']: best_b = {'rmse': r, 'ssim': s, 'rec': rec2d, 't': t, 'n': n_iter}
    print(f"   x{n_iter:3d}: RMSE={r:.5f}, SSIM={s:.4f}, {t*1000:.0f}ms")
print(f"   >> 最优: SIRT x{best_b['n']}: RMSE={best_b['rmse']:.5f}")

# ---- C. FBP + OS-SART ----
print("-" * 55); print("C. FBP + OS-SART (20子集)"); print("-" * 55)
c_hist = []; best_c = {'rmse': 1e9, 'ssim': -1, 'rec': None, 't': 0, 'n': 0}
rec_c = rec_fdk.copy()
os_iters = [1, 2, 5, 10]
for n_iter in os_iters:
    t0 = time(); rec_c = algs.ossart(sino, geo, angles, niter=n_iter, init=rec_c, blocksize=9, verbose=False)
    t = time() - t0; rec2d = linear_scale(rec_c)
    r, s = calc_rmse(rec2d), calc_ssim(rec2d)
    c_hist.append((n_iter, t, r, s))
    if r < best_c['rmse']: best_c = {'rmse': r, 'ssim': s, 'rec': rec2d, 't': t, 'n': n_iter}
    print(f"   x{n_iter:3d}: RMSE={r:.5f}, SSIM={s:.4f}, {t*1000:.0f}ms")
print(f"   >> 最优: OS-SART x{best_c['n']}: RMSE={best_c['rmse']:.5f}")

# ---- D. TV-OS-SART (OS-SART + TV) ----
print("-" * 55); print("D. TV-OS-SART (OS-SART + TV 去噪)"); print("-" * 55)
d_hist = []; best_d = {'rmse': 1e9, 'ssim': -1, 'rec': None, 't': 0, 'n': 0}
rec_d = rec_fdk.copy()
beta = 0.0005
for n_iter in os_iters:
    t0 = time()
    rec_d = algs.ossart(sino, geo, angles, niter=n_iter, init=rec_d, blocksize=9, verbose=False)
    rec_d = rec_d - beta * tv_gradient(rec_d)
    t = time() - t0; rec2d = linear_scale(rec_d)
    r, s = calc_rmse(rec2d), calc_ssim(rec2d)
    d_hist.append((n_iter, t, r, s))
    if r < best_d['rmse']: best_d = {'rmse': r, 'ssim': s, 'rec': rec2d, 't': t, 'n': n_iter}
    print(f"   x{n_iter:3d}: RMSE={r:.5f}, SSIM={s:.4f}, {t*1000:.0f}ms")
...
print(f"{'SIRT x'+str(best_b['n']):30s} {best_b['t']*1000:>8.0f} ms {best_b['rmse']:>12.5f} {best_b['ssim']:>8.4f} {s_imp:>10s}")
print(f"{'OS-SART x'+str(best_c['n']):30s} {best_c['t']*1000:>8.0f} ms {best_c['rmse']:>12.5f} {best_c['ssim']:>8.4f} {c_imp:>10s}")
print(f"{'TV-OS-SART x'+str(best_d['n']):30s} {best_d['t']*1000:>8.0f} ms {best_d['rmse']:>12.5f} {best_d['ssim']:>8.4f} {d_imp:>10s}")
...
for name, t, rmse, ssim in results:
    if 'FDK' in name: rec = fdk_rec
    elif 'SIRT' in name and 'TV' not in name: rec = best_b['rec']
    elif 'OS-SART' in name and 'TV' not in name: rec = best_c['rec']
    elif 'TV-OS' in name: rec = best_d['rec']
...
for i, (title, img, rmse, ssim, t) in enumerate(all_data):
    ...
    tstr = title if rmse is None else f"{title}\nRMSE={rmse:.5f} SSIM={ssim:.4f}\n{t*1000:.0f}ms"
    ax.set_title(tstr, fontsize=7); ax.axis('off')
    ...
    im = ax2.imshow(e, cmap='RdBu_r', vmin=-v, vmax=v); ax2.set_title('Error', fontsize=8); ax2.axis('off')
plt.suptitle('Cone-beam CBCT: TIGRE CUDA (512x512x32)', fontsize=13, fontweight='bold', y=0.98)
plt.savefig("img_out/tigre_cone_hybrid.png", dpi=150, bbox_inches='tight')
plt.close(); print("   => img_out/tigre_cone_hybrid.png")
