...
cfg['ProjectionDataId'] = sid; cfg['ReconstructionDataId'] = rid
cfg['option'] = {'FilterType': 'ram-lak'}
aid = astra.algorithm.create(cfg)
astra.algorithm.run(aid)
astra.algorithm.delete(aid)
astra.data2d.delete(rid); astra.data2d.delete(sid)

# FBP_CUDA
for filt, label in [('ram-lak', 'FBP_CUDA ram-lak(骨)'), ('shepp-logan', 'FBP_CUDA shepp-logan(标准)'), ('hamming', 'FBP_CUDA hamming(平滑)')]:
    t0 = time()
    sid = astra.data2d.create('-sino', proj_geom, sino)
    rid = astra.data2d.create('-vol', vol_geom)
    cfg = astra.astra_dict('FBP_CUDA')
    ...
    t = time() - t0; r = calc_rmse(rec)
    fbp_data.append((label, t, r, rec))
    astra.algorithm.delete(aid); astra.data2d.delete(rid); astra.data2d.delete(sid)
...
for n_iter in [10, 20, 30, 50, 80, 100]:
    ...
    if r < best_r: best_r = r; best_rec = rec; best_t = t; best_n = n_iter
    astra.algorithm.delete(aid); astra.data2d.delete(rid)
...
# ===== 图1: FBP 组 =====
n_fbp = len(fbp_data)
fig1 = make_subplots(rows=2, cols=n_fbp,
    subplot_titles=[f"{fbp_data[i][0]}<br>RMSE={fbp_data[i][2]:.1f}<br>{fbp_data[i][1]*1000:.0f}ms" for i in range(n_fbp)],
...
for i in range(n_fbp):
    ...
    fig1.add_trace(go.Heatmap(z=rec, colorscale=gray_scale, zmin=-200, zmax=600, showscale=False), row=1, col=i+1)
    fig1.add_trace(go.Heatmap(z=diff, colorscale=rdbu_scale, zmin=-vmax, zmax=vmax, showscale=False), row=2, col=i+1)
...
# 收敛曲线 (CGLS + SIRT)
fig2.add_trace(go.Scatter(x=[d[0] for d in ir_data], y=[d[2] for d in ir_data],
    mode='lines+markers', name='CGLS', line=dict(color='purple', width=2), marker=dict(size=8)), row=1, col=1)
fig2.add_trace(go.Scatter(x=[d[0] for d in sirt_data], y=[d[2] for d in sirt_data],
    mode='lines+markers', name='SIRT', line=dict(color='orange', width=2), marker=dict(size=8)), row=1, col=1)
...
fig2.add_trace(go.Heatmap(z=best_c[3], colorscale=gray_scale, zmin=-200, zmax=600, showscale=False), row=1, col=2)
...
fig2.add_trace(go.Heatmap(z=best_c_diff, colorscale=rdbu_scale, zmin=-vmax_c, zmax=vmax_c, showscale=False), row=1, col=3)
...
fig2.add_trace(go.Heatmap(z=best_s_diff, colorscale=rdbu_scale, zmin=-vmax_s, zmax=vmax_s, showscale=False), row=1, col=4)
...
for col, ni in enumerate([10, 30, 100]):
    for d in ir_data:
        if d[0] == ni:
            ...
            fig2.add_trace(go.Heatmap(z=diff, colorscale=rdbu_scale, zmin=-vmax_d, zmax=vmax_d, showscale=False,
                name=f'CGLS x{ni} err'), row=2, col=col+1)
            fig2.update_xaxes(visible=False, row=2, col=col+1)
            fig2.update_yaxes(visible=False, row=2, col=col+1, autorange='reversed')
            fig2.add_annotation(text=f'CGLS x{ni} RMSE={d[2]:.2f}',
                x=0.5, y=-0.15, showarrow=False, font=dict(size=10),
                xref=f'x{col+5}', yref='y5', bgcolor='rgba(255,255,255,0.7)')
            break
...
for name, t, r, _ in results:
    ...
    fig3.add_trace(go.Scatter(x=[t*1000], y=[r], mode='markers+text', name=name,
        text=[name[:18]], textposition='top center',
